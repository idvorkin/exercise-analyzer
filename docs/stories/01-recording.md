...
- **Status:** implemented in [1fb9b0a](https://github.com/idvorkin/exercise-analyzer/commit/1fb9b0a), [adc537a](https://github.com/idvorkin/exercise-analyzer/commit/adc537a), [8bc27b5](https://github.com/idvorkin/exercise-analyzer/commit/8bc27b5); verified on the host (fixtures for all four exercises) and the phone; pull-ups (054, [#108](https://github.com/idvorkin/exercise-analyzer/issues/108)) verified on the host and the simulator, on the phone since 2026-09-19, Igor's check pending; [9b31923](https://github.com/idvorkin/exercise-analyzer/commit/9b31923) fixes #119: full host suite passes, reported clip detects Split Squat 90% with ten frame-reviewed reps and zero setup/rerack reps; installed 2026-09-20; physical-phone log `swing-20260920-101753.jsonl` confirms the reported set re-analyzed as ten split squats and saved after exercise selection, #119 closed; Igor reviewed the Lavish frames with “Looks great”, exact count not explicitly confirmed
...
- **Status:** implemented for [#112](https://github.com/idvorkin/exercise-analyzer/issues/112); verified on the host (`splitsquat-barbell-phone`: detected as split squats, 8 reps; `TuningReports.testSplitSquatTrace`) and the simulator (Igor's clip on Auto: Split Squat, 8 reps); a static split after standing tall (6d617d8 counted 0 of 3) fixed in 836f200 on the host with a stick figure (`SplitSquatAnalyzerTests`), no recorded set of that shape yet; installed on the phone 2026-09-19 (6d617d8, 836f200), Igor's check pending, and the count of 8 is mine, not yet Igor's; fully upright Standing checkpoints [93c65dc](https://github.com/idvorkin/exercise-analyzer/commit/93c65dc) verified on the host against the same phone fixture (eight failing checkpoint assertions now pass; count remains eight; full host suite passes), simulator reopening logged `analyzed` with reason `analyzer_version` and eight reps; simulator and signed phone builds pass; installed and launched on the phone 2026-09-20; Igor's visual check pending; [9b31923](https://github.com/idvorkin/exercise-analyzer/commit/9b31923) fixes #119: full host suite passes, reported clip detects Split Squat 90% with ten frame-reviewed reps and zero setup/rerack reps; installed 2026-09-20; physical-phone log `swing-20260920-101753.jsonl` confirms the reported set re-analyzed as ten split squats and saved after exercise selection, #119 closed; Igor reviewed the Lavish frames with “Looks great”, exact count not explicitly confirmed
