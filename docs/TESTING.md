...
| `SWING_WORKOUT_BAR_TAP=0.58` | with `SWING_SHOW_WORKOUTS=1 SWING_OPEN_WORKOUT=1`: 3 s after the workout's page opens, tap its chart that share of the way across the plot (#101). The log then has `ui workout_bar_tap` with `hit` and, on a hit, `recents_open` with the set's id (the seeded workout's sets sit at 0.32, 0.56 and 0.81) |
