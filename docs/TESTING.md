...
| `SWING_WORKOUT_ZOOM=3` | with `SWING_SHOW_WORKOUTS=1 SWING_OPEN_WORKOUT=1`: 2 s after the page opens, narrow the chart to a third of the workout around the moment 80 % through it, as a pinch would (#125); `ui workout_zoom` logs `window_s`, `whole_s` and `start_s`. With `SWING_WORKOUT_BAR_TAP=0.12` the tap then lands 12 % into that window: the middle seeded set opens, where unzoomed 0.12 hits nothing, which is how the tap is known to go through the scroll |
