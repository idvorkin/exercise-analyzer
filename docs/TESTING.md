...
| `SWING_LIVE_WORKOUT=30` | pretend a workout on the wrist began 30 minutes ago (the simulator has no watch, #123): the app opens on its page (`ui open_live_workout`, then `workout_page` with `live: true`); with `SWING_OPEN_RECENT=<id> SWING_BACK_TO_WORKOUT=1` and enough minutes to cover the set, the set's "‹" opens that page |
